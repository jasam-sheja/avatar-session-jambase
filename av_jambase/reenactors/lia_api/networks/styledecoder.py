...
class ToFlow(nn.Module):
    ...
    def forward(self, input, style, feat, skip=None):
        ...
        # xs = np.stack(xs, 2)
        # xs = torch.from_numpy(xs.astype(np.float32)).to(input.device).unsqueeze(0).repeat(input.size(0), 1, 1, 1)
        xs = torch.linspace(-1, 1, input.size(2), device=input.device)
        xs = torch.meshgrid(xs, xs, indexing='xy')
        xs = torch.stack(xs, 2)
        xs = xs.unsqueeze(0).repeat(input.size(0), 1, 1, 1)
