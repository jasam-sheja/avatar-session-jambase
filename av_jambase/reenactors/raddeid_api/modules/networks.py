...
#===============================================================================
def custom_warp_affine(img, M, dsize):
    B, C, H, W = img.shape  # Get image shape
    device = img.device

    # Create normalized grid
    grid = F.affine_grid(M, [B, C, *dsize], align_corners=False)

    # Apply grid sampling
    warped = F.grid_sample(img, grid, align_corners=False)
    return warped
#===============================================================================
def warp_affine_without_affine_grid(img, M, dsize):
    B, C, H, W = img.shape
    device = img.device

    # Create normalized coordinate grid
    yy, xx = torch.meshgrid(torch.linspace(-1, 1, dsize[0]), torch.linspace(-1, 1, dsize[1]))
    grid = torch.stack((xx, yy), dim=-1).to(device)  # Shape: (H, W, 2)

    # Expand grid for batch processing
    grid = grid.expand(B, -1, -1, -1)

    # Transform grid using the affine matrix
    M_expanded = M[:, :2, :].unsqueeze(1).unsqueeze(1)  # Shape: (B, 1, 1, 2, 3)
    ones = torch.ones(B, dsize[0], dsize[1], 1, device=device)
    grid_homogeneous = torch.cat((grid, ones), dim=-1).unsqueeze(-2)  # (B, H, W, 1, 3)
    transformed_grid = torch.matmul(M_expanded, grid_homogeneous).squeeze(-2)  # (B, H, W, 2)

    # Apply `grid_sample()`
    warped = F.grid_sample(img, transformed_grid, align_corners=False)
    return warped
#===============================================================================
...
class ReconNetWrapper(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.use_last_fc:
            ...
            #x_enc = torch.flatten(torch.cat(output, dim=1), 1)
        return x
