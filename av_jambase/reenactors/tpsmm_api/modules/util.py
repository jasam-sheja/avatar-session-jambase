...
class TPS:
    ...
    def __init__(self, mode, bs, **kwargs):
        ...
        if mode == "random":
            ...
            one1 = torch.ones(
                (self.bs, kp_1.shape[1], kp_1.shape[2], 1), dtype=kp_type, device=device
            )
            kp_1p = torch.cat([kp_1, one1], 3)

            zero = torch.zeros(
                (self.bs, kp_1.shape[1], 3, 3), dtype=kp_type, device=device
            )
            P = torch.cat([kp_1p, zero], 2)
            L = torch.cat([K, kp_1p.permute(0, 1, 3, 2)], 2)
            L = torch.cat([L, P], 3)

            zero = torch.zeros((self.bs, kp_1.shape[1], 3, 2), dtype=kp_type, device=device)
            Y = torch.cat([kp_2, zero], 2)
            one = torch.eye(L.shape[2], dtype=kp_type, device=device).expand(L.shape) * 0.01
            L = L + one
...
def kp2gaussian(kp: Tensor, spatial_size, kp_variance):
    """
...
def make_coordinate_grid(spatial_size, *, dtype=torch.float32, device="cpu"):
    ...
    y = torch.linspace(-1, 1, h, dtype=dtype, device=device)
    xy = torch.meshgrid(x, y, indexing="xy")
    meshed = torch.stack(xy, dim=-1)
    return meshed
